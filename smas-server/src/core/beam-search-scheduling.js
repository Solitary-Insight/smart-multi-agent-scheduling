...
const LOG = {
    ...
    info: (label, msg) => {
        ...
        if (LOG.responseFunction) return console.log(`[INFO] [${label}] ${msg}`);
        console.log(`[INFO] [${label}] ${msg}`);
    },

    step: (label, msg) => {
        if (!LOG.enabled) return;
        if (LOG.responseFunction) return console.log(`[STEP] [${label}] ${msg}`);
        console.log(`[STEP] [${label}] ${msg}`);
    },

    warn: (label, msg) => {
        if (!LOG.enabled) return;
        if (LOG.responseFunction) return console.log(`[WARN] [${label}] ${msg}`);
        console.log(`[WARN] [${label}] ${msg}`);
    },

    error: (label, msg) => {
        if (!LOG.enabled) return;
        if (LOG.responseFunction) return console.log(`[ERROR] [${label}] ${msg}`);
...
function sanitizeOptionsForWorker(options) {
    const { getDB, ...rest } = options || {};
    return JSON.parse(JSON.stringify(rest));
}

function runBeamWorker(payload) {
    return new Promise((resolve, reject) => {
        const worker = new Worker(path.join(__dirname, "beam-search-worker.js"), {
            workerData: payload,
        });
...
async function solveTimetableParallel(userOptions = {}) {
    const DEFAULT_OPTIONS = {
        ...
        onLogResponse: ({ type, message }) => {
            console.log('str', { type, message })
        },
        debug: false,
    };
    ...
    const data = { ...rawData };
    preprocessAvailability(data);
    options.onLogResponse({type:"test",message:"Yeah OK"})
    ...
    const workerCount = Math.max(1, Math.min(cpuCount, restarts));

    const seeds = Array.from({ length: restarts }, (_, i) => 1000 + i * 97);
    const seedBuckets = Array.from({ length: workerCount }, () => []);
    seeds.forEach((seed, i) => {
        seedBuckets[i % workerCount].push(seed);
    });
    ...
    const workerPayloadBase = {
        workItems,
        slots,
        data, // keep as-is; Map/Set are structured-cloneable in worker_threads
    ...
    for (const res of results) {
        if (res.candidateStates?.length) candidateStates.push(...res.candidateStates);

        if (res.validSolutions?.length) {
            for (const s of res.validSolutions) {
...
async function solveTimetable(userOptions = {}) {
    ...
    for (let i = 0; i < restarts; i++) {
        ...
        let msg = { type: "SEARCH", message: `Restart ${i + 1}/${restarts}` }
        if (options.onLogResponse) options.onLogResponse(msg)
        for (const s of states) {
            ...
            validSolutions.push(s);
            msg.type = "SOLUTION"
            msg.message = `Valid solution found (total: ${validSolutions.length})`
            LOG.info(msg.type, msg.message);
...
function beamSearch(workItems, data, slots, options, seed = 1) {
    ...
    for (const item of workItems) {
        const nextStates = [];
        let msg = { type: "GROUP", message: `Placing ${item.group.id}` }
        if (options.onLogResponse) options.onLogResponse(msg)

        LOG.step(msg.type, msg.message);
        ...
        msg.type = "BEAM"
        msg.message = `Completed ${item.group.id} | states: ${states.length}`
        if (options.onLogResponse) options.onLogResponse(msg)
        LOG.info(msg.type, msg.message);
    }
...
// // ─────────────────────────────────────────────────────────────────
// // OCCUPANCY INDEX CONSTANTS
// // Safe upper bounds — no dynamic resizing needed during search
// // ─────────────────────────────────────────────────────────────────
// const OCC_SLOTS_PER_ENTITY = 128; // max unique slot keys (days × time-slots), power-of-2

// // ─────────────────────────────────────────────────────────────────
// // BUILD OCCUPANCY INDEX (called once before beamSearch)
// // Returns compact integer mappings so we can use Uint8Array buffers
// // instead of string-keyed objects for hot-path cloning.
// // ─────────────────────────────────────────────────────────────────
// function buildOccupancyIndex(slots, data) {
//     // slotKey → compact integer index  (max = days × slotsPerDay, well under 128)
//     const slotKeyToIdx = new Map();
//     slots.forEach((s, i) => {
//         slotKeyToIdx.set(`${s.day_id}_${s.startMin}_${s.endMin}`, i);
//     });

//     // teacher_id → compact 0-based index
//     const teacherIdToIdx = new Map();
//     let ti = 0;
//     for (const [id] of data.teacherMap) teacherIdToIdx.set(Number(id), ti++);

//     // room id → compact 0-based index
//     const roomIdToIdx = new Map();
//     let ri = 0;
//     for (const room of data.classrooms) roomIdToIdx.set(Number(room.id), ri++);

//     // student id → compact 0-based index
//     const studentIdToIdx = new Map();
//     let si = 0;
//     // Collect all student IDs from courseStudentsMap
//     for (const [, set] of data.courseStudentsMap) {
//         for (const sid of set) {
//             const n = Number(sid);
//             if (!studentIdToIdx.has(n)) studentIdToIdx.set(n, si++);
//         }
//     }

//     const nSlots = slotKeyToIdx.size;        // actual slot count
//     const nTeachers = teacherIdToIdx.size;
//     const nRooms = roomIdToIdx.size;
//     const nStudents = studentIdToIdx.size;

//     // Buffer lengths: entity_count × OCC_SLOTS_PER_ENTITY
//     // We use OCC_SLOTS_PER_ENTITY (128) as the stride so index maths
//     // stay fast without a multiply-by-nSlots that changes per run.
//     const teacherBufLen = nTeachers * OCC_SLOTS_PER_ENTITY;
//     const roomBufLen = nRooms * OCC_SLOTS_PER_ENTITY;
//     const studentBufLen = Math.max(1, nStudents) * OCC_SLOTS_PER_ENTITY;
...
// const LOG = {
//     enabled: true,
//     info: (label, msg) => { if (LOG.enabled) console.log(`[INFO]  [${label}] ${msg}`); },
//     step: (label, msg) => { if (LOG.enabled) console.log(`[STEP]  [${label}] ${msg}`); },
//     warn: (label, msg) => { if (LOG.enabled) console.log(`[WARN]  [${label}] ${msg}`); },
//     error: (label, msg) => { if (LOG.enabled) console.log(`[ERROR] [${label}] ${msg}`); },
// };
...
//     const edges = new Map();
//     const indegree = new Map();
//     groups.forEach(g => { edges.set(g.id, new Set()); indegree.set(g.id, 0); });
...
// // ─────────────────────────────────────────────────────────────────
// // VALIDATION / SCORING
// // evaluatePlacement — same signature as before, now takes optional
// // `occIdx` (OccupancyIndex) for fast typed-array lookups.
// // When occIdx is absent it falls back to the original string-key logic
// // so the function stays compatible with direct callers (e.g. tests,
// // explainBlockingForItem which passes a plain cloned state).
// // ─────────────────────────────────────────────────────────────────
// function evaluatePlacement(state, group, room, slot, data, options, occIdx) {
//     const violations = [];
//     let penalty = 0;
...
//     // ── Break conflicts ──────────────────────────────────────────
//     for (const br of data.breaks) {
//         const deptMatch = !br.departmentIds || br.departmentIds.size === 0 ||
//             group.departmentIds.some(d => br.departmentIds.has(Number(d)));
//         const dayMatch = !br.dayIds || br.dayIds.size === 0 ||
//             br.dayIds.has(Number(slot.day_id));
//         if (dayMatch && deptMatch && overlaps(slot.startMin, slot.endMin, br.startMin, br.endMin)) {
...
//         // Teacher overlap check — typed-array fast path
//         let teacherOccupied = false;
//         if (occIdx) {
//             const tIdx = occIdx.teacherIdToIdx.get(teacherId);
//             const sIdx = occIdx.slotKeyToIdx.get(slotKey);
//             if (tIdx !== undefined && sIdx !== undefined) {
//                 teacherOccupied = state.teacherOcc[tIdx * OCC_SLOTS_PER_ENTITY + sIdx] === 1;
//             }
//         } else {
//             teacherOccupied = !!state.occupiedTeachers[`${teacherId}_${slotKey}`];
//         }
//         if (teacherOccupied) {
...
//         // Student overlap check — typed-array fast path
//         for (const sid of group.studentIds) {
//             let studentOccupied = false;
//             if (occIdx) {
//                 const stIdx = occIdx.studentIdToIdx.get(Number(sid));
//                 const sIdx = occIdx.slotKeyToIdx.get(slotKey);
//                 if (stIdx !== undefined && sIdx !== undefined) {
//                     studentOccupied = state.studentOcc[stIdx * OCC_SLOTS_PER_ENTITY + sIdx] === 1;
//                 }
//             } else {
//                 studentOccupied = !!state.occupiedStudents[`${sid}_${slotKey}`];
//             }
//             if (studentOccupied) {
...
//     // ── Room overlap — typed-array fast path ─────────────────────
//     const roomId = Number(room.id);
//     let roomOccupied = false;
//     if (occIdx) {
//         const rIdx = occIdx.roomIdToIdx.get(roomId);
//         const sIdx = occIdx.slotKeyToIdx.get(slotKey);
//         if (rIdx !== undefined && sIdx !== undefined) {
//             roomOccupied = state.roomOcc[rIdx * OCC_SLOTS_PER_ENTITY + sIdx] === 1;
//         }
//     } else {
//         roomOccupied = !!state.occupiedRooms[`${roomId}_${slotKey}`];
//     }
//     if (roomOccupied) {
//         violations.push({ code: "room_overlap", message: `Room ${room.id} already occupied at ${slot.start} on day ${slot.day_id}`, room_id: roomId });
//         penalty += 200000;
//     }

//     // ── Group-slot overlap ───────────────────────────────────────
//     const gKey = `${group.id}_${slotKey}`;
//     if (state.groupSlotSet?.has(gKey) || state.groupSlotMap?.[gKey]) {
...
//     if (capacityRuleOn && roomCap > 0) {
//         if (demand > roomCap) {
//             violations.push({ code: "capacity_insufficient", message: `Room ${room.id} capacity ${roomCap} < class size ${demand}`, room_id: roomId, capacity: roomCap, demand });
//             penalty += 120000 + (demand - roomCap) * 1000;
//         }
//         if (options.maxClassSize && demand > options.maxClassSize) {
...
//     // ── Same-day session spreading ───────────────────────────────
//     // FIX: treat same-day repeat as a near-hard constraint so that
//     // a course with N credit hours is spread across N different days.
//     // We only allow same-day stacking if the course has more sessions
//     // than there are non-holiday days (unavoidable), otherwise the
//     // penalty is high enough that the beam will always prefer spreading.
//     const usedDays = state.groupDayMap[group.id];
//     if (usedDays && usedDays.has(slot.day_id)) {
//         const sessions = group.creditHours || 1;
//         const totalDays = data.days ? data.days.length : 6;
//         if (sessions <= totalDays) {
//             // Spreading is geometrically possible — force it
//             penalty += 500000;
//         } else {
//             // More sessions than available days — soft nudge only
//             penalty += 25;
//         }
//     }
...
//     return { valid: !hardViolated, score: penalty, violations, slotKey, roomKey: `${roomId}_${slotKey}` };
// }

// // ─────────────────────────────────────────────────────────────────
// // TOPOLOGICAL SORT / GROUP ORDERING
// // ─────────────────────────────────────────────────────────────────
// function topoSortGroups(groups, prerequisiteRows) {
//     const { edges, indegree } = buildGroupPrereqGraph(groups, prerequisiteRows);
//     const groupMap = new Map(groups.map(g => [g.id, g]));
...
//         let teacherOk = true;
//         for (const course of group.courses) {
//             if (!teacherIsAvailable(data, Number(course.teacher_id), slot.day_id)) { teacherOk = false; break; }
//         }
//         if (!teacherOk) continue;
//         let roomOk = 0;
//         for (const room of data.classrooms) {
//             const cap = Number(room.capacity || 0);
//             if (!options.applyRoomConstraint || !cap || group.demand <= cap) roomOk++;
//         }
//         score += roomOk;
//     }
//     return 1 / (score + 1) + group.teacherIds.length * 2 + group.creditHours;
// }
...
// // ─────────────────────────────────────────────────────────────────
// // STATE — now uses Uint8Array buffers for hot occupancy tracking
// // ─────────────────────────────────────────────────────────────────
// function initialState(occIdx) {
//     if (!occIdx) {
//         return {
//             assignments:      [],
//             occupiedTeachers: {},
...
//     return {
//         assignments:       [],
//         teacherOcc:        new Uint8Array(occIdx.teacherBufLen),
//         roomOcc:           new Uint8Array(occIdx.roomBufLen),
//         studentOcc:        new Uint8Array(occIdx.studentBufLen),
//         groupSlotSet:      new Set(),
//         groupDayMap:       {},
//         courseWeeklyCount: {},
//         score:             0,
//         hardViolations:    0,
...
// // ─────────────────────────────────────────────────────────────────
// // CLONE STATE — the hot path, now 10-20× faster
// // ─────────────────────────────────────────────────────────────────
...
//         const next = {
//             assignments:       state.assignments.slice(),
//             teacherOcc:        state.teacherOcc.slice(),
//             roomOcc:           state.roomOcc.slice(),
//             studentOcc:        state.studentOcc.slice(),
//             groupSlotSet:      new Set(state.groupSlotSet),
//             groupDayMap:       {},
//             courseWeeklyCount: { ...state.courseWeeklyCount },
//             score:             state.score,
//             hardViolations:    state.hardViolations,
//             scheduledCount:    state.scheduledCount,
//             unscheduled:       state.unscheduled.slice(),
...
// // ─────────────────────────────────────────────────────────────────
// // APPLY PLACEMENT — updates typed-array buffers in-place on clone
// // ─────────────────────────────────────────────────────────────────
// function applyPlacement(state, group, room, slot, evalResult, occIdx) {
//     const next    = cloneState(state);
//     const slotKey = `${slot.day_id}_${slot.startMin}_${slot.endMin}`;

//     next.score          += evalResult.score;
//     next.hardViolations += evalResult.violations.filter(v =>
//         ["break_conflict","teacher_unavailable","teacher_overlap",
//          "room_overlap","student_overlap","capacity_insufficient","group_overlap"]
//         .includes(v.code)
//     ).length;
//     next.scheduledCount += 1;
...
//     if (occIdx && next.teacherOcc) {
//         const sIdx = occIdx.slotKeyToIdx.get(slotKey);
//         if (sIdx !== undefined) {
//             for (const course of group.courses) {
//                 const tIdx = occIdx.teacherIdToIdx.get(Number(course.teacher_id));
//                 if (tIdx !== undefined) next.teacherOcc[tIdx * OCC_SLOTS_PER_ENTITY + sIdx] = 1;
//             }
//             const rIdx = occIdx.roomIdToIdx.get(Number(room.id));
//             if (rIdx !== undefined) next.roomOcc[rIdx * OCC_SLOTS_PER_ENTITY + sIdx] = 1;
//             for (const sid of group.studentIds) {
//                 const stIdx = occIdx.studentIdToIdx.get(Number(sid));
//                 if (stIdx !== undefined) next.studentOcc[stIdx * OCC_SLOTS_PER_ENTITY + sIdx] = 1;
//             }
//         }
//         next.groupSlotSet.add(`${group.id}_${slotKey}`);
//     } else {
//         for (const course of group.courses) {
//             next.occupiedTeachers[`${Number(course.teacher_id)}_${slotKey}`] = true;
//         }
//         next.occupiedRooms[`${Number(room.id)}_${slotKey}`] = true;
//         for (const sid of group.studentIds) {
//             next.occupiedStudents[`${sid}_${slotKey}`] = true;
...
//     // ── CREDIT TOKEN: decrement one token per course in this group ──
//     for (const cid of group.courseIds) {
//         next.courseWeeklyCount[cid] = (next.courseWeeklyCount[cid] || 0) + 1;
//     }
...
// // ─────────────────────────────────────────────────────────────────
// // ENUMERATE CANDIDATES (optimized — skips impossible slots early)
// // ─────────────────────────────────────────────────────────────────
// function enumerateCandidates(state, group, data, slots, options, rng, occIdx) {

//     // ── Credit token guard ───────────────────────────────────────
//     // If every course in this group has already consumed all its
//     // weekly sessions, skip candidate generation entirely.
//     for (const course of group.courses) {
//         const cid   = Number(course.id);
//         const limit = Number(course.credit_hours || 0);
//         const used  = state.courseWeeklyCount?.[cid] || 0;
...
//             const evalResult = evaluatePlacement(state, group, room, slot, data, options, occIdx);
...
// // ─────────────────────────────────────────────────────────────────
// // BEAM SEARCH — now builds occupancy index once, threads it through
// // ─────────────────────────────────────────────────────────────────
// function beamSearch(workItems, data, slots, options, seed = 1) {
//     const rng = mulberry32(seed);
//     // Build compact index once — O(n) setup, saves O(n) per clone
//     const occIdx = buildOccupancyIndex(slots, data);
//     let states = [initialState(occIdx)];

//     for (const item of workItems) {
//         const nextStates = [];
//         const msg = { type: "GROUP", message: `Placing ${item.group.id}` };
//         if (options.onLogResponse) options.onLogResponse(msg);
//         LOG.step(msg.type, msg.message);

//         for (const state of states) {
//             const candidates = enumerateCandidates(state, item.group, data, slots, options, rng, occIdx);
...
//             for (const cand of candidates) {
//                 const placed = applyPlacement(state, item.group, cand.room, cand.slot, cand.evalResult, occIdx);
//                 nextStates.push(placed);
//             }
//         }
...
// function runBeamWorker(payload) {
//     return new Promise((resolve, reject) => {
//         const worker = new Worker(path.join(__dirname, "beam-search-worker.js"), { workerData: payload });
//         worker.on("message", (msg) => {
//             if (msg && msg.error) { reject(new Error(msg.error + (msg.stack ? `\n${msg.stack}` : ""))); return; }
//             resolve(msg);
//         });
//         worker.on("error", reject);
...
// // ─────────────────────────────────────────────────────────────────
// // MAIN SOLVERS — unchanged signatures, unchanged return shapes
// // ─────────────────────────────────────────────────────────────────
// async function solveTimetableParallel(userOptions = {}) {
//     const DEFAULT_OPTIONS = {
//         nSolutions: 5,
...
//         enforceTeacherDepartment: false,
//         usePrerequisiteOrdering: true,
//         onLogResponse: ({ type, message }) => { console.log("str", { type, message }); },
//         debug: false,
//     };
...
//     const seeds = Array.from({ length: restarts }, (_, i) => 1000 + i * 97);
//     const seedBuckets = Array.from({ length: workerCount }, () => []);
//     seeds.forEach((seed, i) => seedBuckets[i % workerCount].push(seed));

//     const workerOptions = sanitizeOptionsForWorker(options);
//     const workerPayloadBase = { workItems, slots, data, options: workerOptions };

//     const results = await Promise.all(
//         seedBuckets
//             .filter(bucket => bucket.length > 0)
//             .map(bucket => runBeamWorker({ ...workerPayloadBase, seeds: bucket }))
//     );
...
//     const solutions = finalizeSolutions(pool, data, slots, options, Number(options.nSolutions || 5));

//     return { config, options, preflightIssues, slotsGenerated: slots.length, totalWorkItems: workItems.length, validSolutionCount: validSolutions.length, solutions };
// }
...
//             msg = { type: "SOLUTION", message: `Valid solution found (total: ${validSolutions.length})` };
//             LOG.info(msg.type, msg.message);
//             if (options.onLogResponse) options.onLogResponse(msg);

//             if (options.stopOnFirstValid && validSolutions.length >= options.nSolutions) break outer;
//         }
//     }
...
// async function getSemesterConfig({ getDB }) {
//     const db = getDB();
//     const [rows] = await db.execute("SELECT value FROM configurations WHERE `key` = 'SEMESTER_CONFIGURATIONS' LIMIT 1");
//     if (!rows.length) throw new Error("Missing SEMESTER_CONFIGURATIONS");
//     const config = JSON.parse(rows[0].value);
//     return {
//         ...config,
...
//     const [mergePartners] = await db.execute("SELECT * FROM course_merge_partners");
//     const [coursePrerequisites] = await db.execute("SELECT * FROM course_prerequisites");
//     const [studentCourseRows] = await db.execute("SELECT student_id, course_id, status FROM student_courses WHERE status = 'enrolled'");
//     const [studentCreditRows] = await db.execute(`
//       SELECT sc.student_id, SUM(c.credit_hours) AS total_credits
//       FROM student_courses sc JOIN courses c ON c.id = sc.course_id
//       WHERE sc.status = 'enrolled' GROUP BY sc.student_id
//     `);
...
//     const teacherDepartmentsMap = new Map();
//     teacherDepartments.forEach(row => {
//         const tid = Number(row.teacher_id), did = Number(row.department_id);
//         if (!teacherDepartmentsMap.has(tid)) teacherDepartmentsMap.set(tid, new Set());
//         teacherDepartmentsMap.get(tid).add(did);
//     });
...
//     const courseStudentsMap = new Map();
//     studentCourseRows.forEach(row => {
//         const cid = Number(row.course_id), sid = Number(row.student_id);
//         if (!courseStudentsMap.has(cid)) courseStudentsMap.set(cid, new Set());
//         courseStudentsMap.get(cid).add(sid);
//     });

//     const studentCreditMap = new Map();
//     studentCreditRows.forEach(row => studentCreditMap.set(Number(row.student_id), Number(row.total_credits || 0)));

//     const breakDayMap = new Map();
//     breakDays.forEach(row => {
//         const bid = Number(row.break_id), did = Number(row.day_id);
//         if (!breakDayMap.has(bid)) breakDayMap.set(bid, new Set());
//         breakDayMap.get(bid).add(did);
//     });
...
//     const breakDeptMap = new Map();
//     breakDepartments.forEach(row => {
//         const bid = Number(row.break_id), did = Number(row.department_id);
//         if (!breakDeptMap.has(bid)) breakDeptMap.set(bid, new Set());
//         breakDeptMap.get(bid).add(did);
//     });
...
//     const mergePartnerMap = new Map();
//     mergePartners.forEach(p => {
//         const mid = Number(p.merge_id), cid = Number(p.course_id);
//         if (!mergePartnerMap.has(mid)) mergePartnerMap.set(mid, new Set());
//         mergePartnerMap.get(mid).add(cid);
//     });
...
//     return {
//         courses, teachers, teacherMap, teacherDepartmentsMap,
//         classrooms, days,
//         availability: availability.map(a => ({ teacher_id: Number(a.teacher_id), day_id: Number(a.day_id) })),
//         breaks: breaks.map(b => ({
//             id: Number(b.id), label: b.label,
//             startMin: timeToMinutes(b.start),
//             endMin: timeToMinutes(b.end),
//             dayIds: breakDayMap.get(Number(b.id)) || new Set(),
//             departmentIds: breakDeptMap.get(Number(b.id)) || new Set(),
//         })),
//         mergeDefs, mergePartnerMap, mergeNameMap,
//         prerequisiteRows, courseById,
//         courseStudentsMap, studentCreditMap,
//     };
// }

// // ─────────────────────────────────────────────────────────────────
// // PREFLIGHT CHECKS (unchanged)
// // ─────────────────────────────────────────────────────────────────
// function preflightChecks(data, config, options) {
//     const issues = [];
//     const maxRoomCapacity = Math.max(...data.classrooms.map(r => Number(r.capacity || 0)), 0);
...
//         if (config.maxClassSize && demand > config.maxClassSize)
//             issues.push({ type: "preflight", code: "class_too_large", course_id: cid, course_name: course.course_name, course_code: course.course_code, student_count: demand, maxClassSize: config.maxClassSize });

//         if (options.applyRoomConstraint && demand > maxRoomCapacity)
//             issues.push({ type: "preflight", code: "no_room_big_enough", course_id: cid, course_name: course.course_name, course_code: course.course_code, student_count: demand, maxRoomCapacity });

//         const neverAvailable = [1, 2, 3, 4, 5, 6, 7].every(d => !teacherIsAvailable(data, teacherId, d));
//         if (neverAvailable)
//             issues.push({ type: "preflight", code: "teacher_no_availability_found", course_id: cid, course_name: course.course_name, course_code: course.course_code, teacher_id: teacherId });

//         if (options.enforceTeacherDepartment && !teacherHasDepartment(data, teacherId, deptId))
//             issues.push({ type: "preflight", code: "teacher_department_mismatch", course_id: cid, course_name: course.course_name, course_code: course.course_code, teacher_id: teacherId, department_id: deptId });
//     }

//     for (const [studentId, totalCredits] of data.studentCreditMap.entries()) {
//         if (config.maxCredits && totalCredits > config.maxCredits)
//             issues.push({ type: "preflight", code: "student_over_max_credits", student_id: studentId, total_credits: totalCredits, maxCredits: config.maxCredits });
//         if (config.minCredits && totalCredits < config.minCredits)
//             issues.push({ type: "preflight", code: "student_below_min_credits", student_id: studentId, total_credits: totalCredits, minCredits: config.minCredits });
//     }
...
//             const uniqCredits = [...new Set(g.courseCreditHours)];
//             if (uniqCredits.length > 1)
//                 issues.push({ type: "preflight", code: "merge_credit_mismatch", group_id: g.id, merge_name: g.mergeName, course_ids: g.courseIds, course_names: g.courseNames, credit_hours: g.courseCreditHours });
//         }
//     }
//     return issues;
// }
...
// // ─────────────────────────────────────────────────────────────────
// // GROUP BUILDING / WORK ITEM EXPANSION (unchanged)
// // ─────────────────────────────────────────────────────────────────
// function buildCourseGroups(data) {
//     const used = new Set();
//     const groups = [];
...
//         let mergeId = null;
//         for (const [mid, ids] of data.mergePartnerMap.entries()) {
//             if (ids.has(cid)) { mergeId = mid; break; }
//         }
...
//             courses.forEach(c => used.add(Number(c.id)));
//             // Mark ALL ids in the merge as used, even if courseById didn't have them.
//             // Without this, a missing course leaks into the single-course branch below
//             // and gets its own extra work items.
//             ids.forEach(id => used.add(Number(id)));
//             const mergeName = data.mergeNameMap.get(mergeId) || `merge_${mergeId}`;
//             const students = new Set();
//             const teacherIds = new Set();
//             const departmentIds = new Set();
//             const courseNames = [], courseCodes = [], creditHoursList = [];
...
//             groups.push({
//                 id: `merge_${mergeId}`, type: "combined", mergeId, mergeName, courses,
//                 courseIds: courses.map(c => Number(c.id)),
...
//             groups.push({
//                 id: `course_${cid}`, type: "single", mergeId: null, mergeName: null,
//                 courses: [course],
//                 courseIds: [cid],
//                 courseNames: [course.course_name],
//                 courseCodes: [course.course_code],
...
// // ─────────────────────────────────────────────────────────────────
// // ANALYSIS / ISSUE REPORTING (unchanged interface)
// // ─────────────────────────────────────────────────────────────────
// function summarizeCounts(obj, limit = 5) {
//     return Object.entries(obj)
//         .map(([k, v]) => ({ key: k, count: v }))
...
// function explainBlockingForItem(state, item, data, slots, options) {
//     const group = item.group;
//     const reasonCounts = {}, blockedStudents = {}, blockedTeachers = {}, blockedRooms = {};
//     let feasiblePlacements = 0, bestNearMiss = null;

//     for (const slot of slots) {
//         for (const room of data.classrooms) {
//             // Uses legacy path (no occIdx) — this is called on already-finished states
//             const evalResult = evaluatePlacement(state, group, room, slot, data, options);
//             if (evalResult.valid) {
//                 feasiblePlacements++;
//             } else {
...
//                 }
//                 if (!bestNearMiss || evalResult.score < bestNearMiss.score) {
//                     bestNearMiss = { room_id: Number(room.id), room_name: room.name, day_id: Number(slot.day_id), day_name: slot.day_name, start_time: slot.start, end_time: slot.end, score: evalResult.score, violations: evalResult.violations.map(v => v.code) };
//                 }
//             }
//         }
//     }
...
//     return {
//         group_id: group.id, label: group.type, merge_name: group.mergeName || null,
//         course_ids: group.courseIds, course_names: group.courseNames,
//         teacher_ids: group.teacherIds, department_ids: group.departmentIds,
//         student_count: group.demand, feasiblePlacements,
//         reasons: summarizeCounts(reasonCounts, 8),
//         topStudentBlockers: summarizeCounts(blockedStudents, 5),
//         topTeacherBlockers: summarizeCounts(blockedTeachers, 5),
//         topRoomBlockers: summarizeCounts(blockedRooms, 5),
...
//     for (const item of state.unscheduled) {
//         issues.push({ type: "unscheduled", ...explainBlockingForItem(state, item, data, slots, options) });
//     }
...
//         issues.push({
//             type: "conflict",
//             group_id: a.group_id, label: a.label, merge_name: a.merge_name,
//             course_ids: a.course_ids, course_names: a.course_names,
//             teacher_ids: a.teacher_ids, department_ids: a.department_ids,
//             room_id: a.classroom_id, day_id: a.day_id,
//             start_time: a.start_time, end_time: a.end_time,
//             violations: a.violations, penalty: a.penalty,
//         });
//     }
//     return issues;
// }
...
//         unique.push({
//             complete, score: state.score, hardViolations: state.hardViolations,
//             scheduledCount: state.scheduledCount,
//             totalWorkItems: state.scheduledCount + state.unscheduled.length,
//             unscheduledCount: state.unscheduled.length,
//             timetable, issues,
...
// // ─────────────────────────────────────────────────────────────────
// module.exports = {
//     solveTimetable, solveTimetableParallel, orderGroups,
//     getSemesterConfig, loadData, preflightChecks,
//     buildCourseGroups, expandWorkItems, generateSlots,
//     beamSearch, enumerateCandidates, evaluatePlacement, applyPlacement, cloneState, initialState,
//     sortByStableScore, candidateSortKey, mulberry32,
//     topoSortGroups, buildGroupPrereqGraph, estimateGroupDifficulty, buildTeacherNameMap, preprocessAvailability,
//     timeToMinutes, minutesToTime, overlaps, preferredPenalty, slotAppliesToBreak,
//     teacherIsAvailable, teacherHasDepartment,
//     summarizeCounts, explainBlockingForItem, buildSolutionIssues,
//     buildTimetableFromState, finalizeSolutions,
//     isValidFinalState, buildStateSignature,
//     // New — exported so beam-search-worker.js can use the same fast path
//     buildOccupancyIndex,
// };

// // docs
// // FUNCTION solveTimetable:
// //   load config → load data → generate slots → build groups → order groups → expand sessions
// //   FOR each restart: states = beamSearch() → collect valid solutions
// //   sort all states → pick best solutions → RETURN